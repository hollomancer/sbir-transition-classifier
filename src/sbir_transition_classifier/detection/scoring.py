...
class ConfigurableScorer:
    ...
    def calculate_likelihood_score(self, sbir_award: Dict[str, Any], contract: Dict[str, Any]) -> float:
        """
    ...
    def is_within_timing_window(self, sbir_award: Dict[str, Any], contract: Dict[str, Any]) -> bool:
        """
    ...
    def _agencies_match(self, sbir_award: Dict[str, Any], contract: Dict[str, Any]) -> bool:
    ...
    def _calculate_timing_score(self, sbir_award: Dict[str, Any], contract: Dict[str, Any]) -> float:
    ...
    def _calculate_text_similarity(self, sbir_award: Dict[str, Any], contract: Dict[str, Any]) -> float:
        ...
        if isinstance(raw_data, dict):
            ...
            if not contract_desc:
                contract_desc = str(raw_data.get('product_or_service_description', '')).lower()
    ...
    def create_evidence_bundle(self, sbir_award: Dict[str, Any], contract: Dict[str, Any], score: float) -> Dict[str, Any]:
        ...
        evidence = {
            ...
            'scoring_components': {
                ...
                'text_similarity': self._calculate_text_similarity(sbir_award, contract) if self.config.detection.features.enable_text_analysis else None
            },
            'timing_analysis': {
                'sbir_completion': completion_date.isoformat() if pd.notna(completion_date) else None,
                'contract_start': start_date.isoformat() if pd.notna(start_date) else None,
                'days_difference': (start_date - completion_date).days if pd.notna(completion_date) and pd.notna(start_date) else None,
