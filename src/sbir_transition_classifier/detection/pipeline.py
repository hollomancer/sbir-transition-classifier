...
class ConfigurableDetectionPipeline:
    ...
    def run_detection(self, sbir_awards: List[Dict[str, Any]], contracts: List[Dict[str, Any]]) -> List[Detection]:
        """
        ...
        logger.info(f"Detection pipeline completed. Found {len(detections)} transitions.")
        return detections
    
    def _process_award(self, award: Dict[str, Any], contracts: List[Dict[str, Any]]) -> List[Detection]:
    ...
    def validate_input_data(self, sbir_awards: List[Dict[str, Any]], contracts: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Validate input data quality."""
        validation_results = {
            'sbir_awards': {
                'total': len(sbir_awards),
                'phase2': len([a for a in sbir_awards if str(a.get('phase', '')).upper() == 'II']),
                'missing_completion_date': len([a for a in sbir_awards if not a.get('completion_date')]),
                'missing_vendor': len([a for a in sbir_awards if not a.get('vendor_name')])
            },
            'contracts': {
                'total': len(contracts),
                'missing_start_date': len([c for c in contracts if not c.get('start_date')]),
                'missing_vendor': len([c for c in contracts if not c.get('vendor_name')]),
