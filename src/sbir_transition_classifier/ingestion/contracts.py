...
class ContractIngester(BaseIngester):
    ...
    def ingest(self, file_path: Path, chunk_size: int = 100000) -> IngestionStats:
        ...
        required_cols = [
            'award_id_piid', 'awarding_agency_name', 'recipient_name',
    ...
    def _process_chunk(self, db: Session, chunk_df: pd.DataFrame, vendor_cache: dict, chunk_num: int):
        ...
        # Track rejections
        missing_piid = (~chunk_df['award_id_piid'].notna()) | (chunk_df['award_id_piid'].str.strip() == '')
        missing_agency = (~chunk_df['awarding_agency_name'].notna()) | (chunk_df['awarding_agency_name'].str.strip() == '')
        
        self.stats.rejection_reasons['missing_piid'] = self.stats.rejection_reasons.get('missing_piid', 0) + missing_piid.sum()
        self.stats.rejection_reasons['missing_agency'] = self.stats.rejection_reasons.get('missing_agency', 0) + missing_agency.sum()
        ...
        self.log_progress(f"Chunk {chunk_num}: {len(contracts_data):,} contracts inserted")
    ...
    def _process_vendors(self, db: Session, chunk_df: pd.DataFrame, vendor_cache: dict):
        ...
        if new_recipients:
            existing_vendors = db.query(models.Vendor).filter(
                models.Vendor.name.in_(new_recipients)
            ).all()
