...
@click.option("--overwrite", is_flag=True, help="Overwrite existing file if it exists")
@click.option(
    "--backup/--no-backup",
    default=True,
...
def reset_config(output: Path, template: str, overwrite: bool, backup: bool):
    ...
    try:
        ...
        if template == "default":
            ...
            success = ConfigReset.generate_template(
                template, output, overwrite=overwrite
            )
        ...
        if success:
            ...
            click.echo(
                f"  2. Validate the configuration: sbir-detect validate-config --config {output}"
            )
            click.echo(
                f"  3. Run detection: sbir-detect run --config {output} --output results/"
            )
        else:
...
def list_templates():
    ...
    click.echo("Usage:")
    click.echo(
        "  sbir-detect reset-config --template <name> --output config/my-config.yaml"
    )
...
def show_template(template: str):
    ...
    try:
        from ..config.schema import DefaultConfig
        import yaml
