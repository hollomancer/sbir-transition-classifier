...
from ..data.hygiene import DataCleaner, create_sample_files_robust
...
@hygiene.command()
@click.option('--data-dir', type=click.Path(exists=True), default='./data', help='Data directory')
@click.option('--test-dir', type=click.Path(), default='./test_data', help='Test data output directory')
...
@hygiene.command()
@click.option('--input-file', type=click.Path(exists=True), required=True, help='Input CSV file')
...
def clean_file(input_file: str, output_file: str, sample_size: int):
    ...
    try:
        cleaned_path = cleaner.clean_csv_file_streaming(input_path, output_path, sample_size)
...
@hygiene.command()
@click.option('--test-dir', type=click.Path(), default='./test_data', help='Test data directory')
@click.option('--run-detection', is_flag=True, help='Run detection pipeline after setup')
